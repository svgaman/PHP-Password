...
class PasswordMb {
  ...
  public function validatePassword($password) {
    ...
    if (mb_strlen($password) < $this->minLength) {
      $this->errors[] = sprintf(_('Password must be %d characters long, current password is too short at %d characters.'), $this->minLength, mb_strlen($password));
      return false;
    }
    // Check password maximum length, return at this step.
    if (mb_strlen($password) > $this->maxLength) {
      $this->errors[] = sprintf(_('Password must be %d characters long, current password is too long at %d characters.'), $this->minLength, mb_strlen($password));
      return false;
    }
    // Check the number of numbers in the password.
    if (mb_strlen(preg_replace('/([^0-9]*)/', '', $password)) < $this->minNumbers) {
    ...
    if (mb_strlen(preg_replace('/([^a-zA-Z]*)/', '', $password)) < $this->minLetters) {
    ...
    if (mb_strlen(preg_replace('/([^a-z]*)/', '', $password)) < $this->minLowerCase && $this->minLowerCase != 0) {
    ...
    if (mb_strlen(preg_replace('/([^A-Z]*)/', '', $password)) < $this->minUpperCase && $this->minUpperCase != 0) {
    ...
    if (mb_strlen(preg_replace('/([a-zA-Z0-9]*)/', '', $password)) < $this->minSymbols && $this->maxSymbols != 0) {
    ...
    if (mb_strlen(preg_replace('/([a-zA-Z0-9]*)/', '', $password)) > $this->maxSymbols) {
      ...
      if ($this->maxSymbols != 0) {
          ...
          for ($i = 0; $i < mb_strlen($symbols); ++$i) {
              $symbolRes = false;
              $symbolItem =  mb_substr($symbols, $i, 1);

              foreach ($this->allowedSymbols as $sym) {
                  if ($sym === $symbolItem) {
                      $symbolRes = true;
                      break;
                  }
